...
SFL Lakes Finding the r-bar for each lake
```{r}
...
SFL_Adult_LakeSummary$cell <- as.factor(SFL_Adult_LakeSummary$cell)
...
R1 <- lm(data = SFL_Adult_LakeSummary, mu ~ depth + basin + cell, weights=Weight) #LM on the growth rate of frogs based on lake characteristics, weight is 1/SE^2 as suggested by Bruce.  This is th

summary(R1)


R2 <- lm(data = SFL_Adult_LakeSummary, mu ~ depth + elev + basin, weights = Weight) #LM on the growth rate of frogs based on lake characteristics, weight is 1/SE^2 as suggested by Bruce.  This is th
...
R_Adults <- FrogLake_agg %>% #Select the FrogLake_Agg
  subset(adult != 0) %>% #Remove any datapoints where no frogs were found
  group_by(site) %>%  #group by the site (lake)
  filter(n() > 4) %>% #Remove any sites which were surveyed for 2 or less years times during the 20 years
  summarise(mu = estimate_SEG_params(adult, year)$mu, #calculate the mu for adults
            SE = sqrt(estimate_SEG_params(adult, year)$sigma2/(n()-1))) #calculate the standard error for adults for each site
...
SF_Adult_LakeSummary <- R_Adult_Lake %>% 
  mutate(Weight = 1/(SE^2)) %>%
  group_by(site) %>% 
  subset(fish == "SF") %>% 
  summarise(mu = mean(mu),
...
SF_Adult_LakeSummary$basin <- as.factor(SF_Adult_LakeSummary$basin)
SF_Adult_LakeSummary$cell <- as.factor(SF_Adult_LakeSummary$cell)

SF_Adult_LakeSummary <- SF_Adult_LakeSummary %>% 
  subset(Weight != "Inf") #remove inf weights
#R_lm <- R_Adult_Lake %>% 
  #group_by(site) %>% 
  #summarise
    
RSF <- lm(data = SF_Adult_LakeSummary, mu ~ depth + elev + basin + cell, weights=Weight) #LM on the growth rate of frogs based on lake characteristics, weight is 1/SE^2 as suggested by Bruce.  This is th

summary(RSF)


RSF2 <- lm(data = SF_Adult_LakeSummary, mu ~ depth + elev + basin, weights = Weight) #LM on the growth rate of frogs based on lake characteristics, weight is 1/SE^2 as suggested by Bruce.  This is th
  #agg_form(formula = adult~year+site, FUN = mean) %>% 
...
R_Adults <- FrogLake_agg %>% #Select the FrogLake_Agg
  subset(adult != 0) %>% #Remove any datapoints where no frogs were found
  group_by(site) %>%  #group by the site (lake)
  filter(n() > 5) %>% #Remove any sites which were surveyed for 2 or less years times during the 20 years
  summarise(mu = estimate_SEG_params(adult, year)$mu, #calculate the mu for adults
            SE = sqrt(estimate_SEG_params(adult, year)$sigma2/(n()-1))) #calculate the standard error for adults for each site
...
No_Adult_LakeSummary <- R_Adult_Lake %>% 
  mutate(Weight = 1/(SE^2)) %>% 
  group_by(site) %>% 
  subset(fish == "No") %>% 
  summarise(mu = mean(mu),
...
No_Adult_LakeSummary$basin <- as.factor(No_Adult_LakeSummary$basin)
No_Adult_LakeSummary$cell <- as.factor(No_Adult_LakeSummary$cell)

No_Adult_LakeSummary <- No_Adult_LakeSummary %>% 
  subset(Weight != "Inf") #remove inf weights
#R_lm <- R_Adult_Lake %>% 
  #group_by(site) %>% 
  #summarise
    
RNo1 <- lm(data = No_Adult_LakeSummary, mu ~ depth + elev + basin + cell, weights=Weight) #LM on the growth rate of frogs based on lake characteristics, weight is 1/SE^2 as suggested by Bruce.  This is th

summary(RNo1)


RNo2 <- lm(data = No_Adult_LakeSummary, mu ~ depth + elev + basin, weights = Weight) #LM on the growth rate of frogs based on lake characteristics, weight is 1/SE^2 as suggested by Bruce.  This is th
  #agg_form(formula = adult~year+site, FUN = mean) %>% 
