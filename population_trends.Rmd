...
Lakes_Adult <- aggregate(adult~site, SFLAdult, sum) %>% 
  subset(adult != 0)
...
R_Adult_Lake$basin <- as.factor(R_Adult_Lake$basin)

R_Adult_Lake <- R_Adult_Lake %>% 
...
R1 <- lm(data = R_Adult_Lake, mu ~ depth + elev + basin + fish, weights=Weight) #LM on the growth rate of frogs based on lake characteristics, weight is 1/SE^2 as suggested by Bruce.  This is th
